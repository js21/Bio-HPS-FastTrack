...
ok( my $update_runner = Bio::HPS::FastTrack::PipelineRun::Update->new( study => 'MRSA studies', lane => '15360_1#1', database => 'pathogen_hpsft_test', mode => 'test' ), 'Creating an Update runner object');
isa_ok ( $update_runner, 'Bio::HPS::FastTrack::PipelineRun::Update' );
is ( $update_runner->command_to_run(),
     q(/software/pathogen/projects/update_pipeline/bin/update_pipeline.pl -n 'MRSA studies' --database=pathogen_hpsft_test -run 15360 -min 15359 -l t/data/conf/fast_track/.pathogen_hpsft_test.update_pipeline.lock -nop -v --file_type cram),
     'Update command to run');
is ( $update_runner->lock_file, 't/data/conf/fast_track/.pathogen_hpsft_test.update_pipeline.lock', 'Lock file string');
...
is ( $update_runner2->lock_file, 't/data/conf/fast_track/.pathogen_hpsft_test.update_pipeline.lock', 'Lock file string');
